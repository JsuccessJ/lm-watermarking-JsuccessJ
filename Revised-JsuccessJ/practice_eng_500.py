...
# 입력 파일에서 데이터 읽기
input_file = '/data/jaesunghwang/lm-watermarking/practice/full_cnndm_ref_val_noindex.txt'
...
with open(output_file, 'w') as out_file:
    ...
    for idx, line in tqdm(enumerate(input_lines[:500]), total=500, desc="Generating texts"):
        ...
        # JSON 형식으로 저장
        json.dump({"Generated": output_text}, out_file)
        out_file.write('\n')

print("Generated texts saved to 'output_practice_eng_500.json'")
...
# 각 문장에서 워터마크 탐지
with open('detection_results_eng500.json', 'w') as detect_file:
    for idx, entry in tqdm(enumerate(watermarked_texts), total=len(watermarked_texts), desc="Detecting watermarks"):
        ...
        json.dump({"line": idx + 1, "detection_result": score_dict}, detect_file)
        detect_file.write('\n')

print("Watermark detection results saved to 'detection_results.json'")
